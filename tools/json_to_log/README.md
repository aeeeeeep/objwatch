...
- `-o, --output <output_file>`: (Optional) Path to the output log file
  - If not specified, the tool will create a log file with the same base name as the input JSON file but with a `.log` extension

### Example

```bash
# Convert objwatch.json to objwatch.log
python3 json_to_log.py objwatch.json

# Convert objwatch.json to custom_output.log
