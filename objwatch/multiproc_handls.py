...
from .utils.logger import create_logger, switch_logger, log_error, log_info
...
class MultiProcHandls:
    ...
    def sync(self) -> None:
        """
        Synchronizes processes across all indexes.
        """
    ...
    def create_multi_logger(
        self, output: Optional[str] = None, level: Union[int, str] = logging.DEBUG, simple: bool = False
    ) -> None:
        if self.index in self.indexes:
            multi_name = f"{self.framework}-{self.index}"
            multi_output = os.path.join(os.path.dirname(output), f"{multi_name}.log") if output is not None else None
            create_logger(name=multi_name, output=multi_output, level=level, simple=simple)
            switch_logger(multi_name)
