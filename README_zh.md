...
- `ranks`（列表，可选）：在使用 `torch.distributed` 时跟踪的 GPU 排序。
...
ObjWatch 无缝集成到分布式 PyTorch 程序中，允许你跨多个 GPU 监控和追踪操作。使用 `ranks` 参数指定要跟踪的 GPU 排序。
...
class CustomWrapper(FunctionWrapper):
 ...
 wrap_call(self, func_name, frame):
        return f" - 调用 {func_name}，参数：{frame.f_locals}"

    def wrap_return(self, func_name, result):
        return f" - {func_name} 返回结果：{result}"
